...
def run():
    """
    Connects to the database, searches for any table containing 'roads',
    ...
    if not tables:
        raise ValueError("❌ No tables found containing 'road'.")

    roads_table = tables[0]  # Use the first matching table

    # Step 2: Read entire table using GeoPandas
    sql = f"SELECT * FROM {roads_table};"
    ...
    print(f"✅ Loaded {len(gdf)} rows from '{roads_table}' as GeoDataFrame.")
    return gdf
